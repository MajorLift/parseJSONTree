...
function JSONParser(input: string): Record<string, unknown> | any[] | string | number | boolean | null {
  ...
  function parsePrimitives (string: string | any): string | number | boolean | null  {
  ...
  function parseObject (string: string): Record<string, Record<string, unknown> | any[] | string | number | boolean> {
    return string === '{}' ? {} 
      : Object.fromEntries(
  ...
  function traverseJSONTree (string: string): any[] | Record<string, Record<string, unknown> | any[] | string | number | boolean> {
    ...
    for (let i = string.length - 1; i >= 0; i--) {
      ...
      if (['[','{'].includes(s)) { // assumes valid bracket pairing throughout input
        ...
        const [start, end] = [i, bracketStack.pop() as number + 1];
